...
| --- | --- | --- | --- | --- |
| Market data | `MarketDataProvider` | Financial Modeling Prep | `mock`, `rest` | REST client written, untested against a live key |
| Options market | `OptionsMarketProvider` | Robinhood | `mock`, `mcp` | MCP adapter written, needs a runtime with tool access |
...
| --- | --- | --- | --- |
| Current quote | `get_quote` | `/api/v3/quote/{symbol}` | Agents 1–3, technicals, price reconciliation |
| Historical prices and volume | `get_price_history` | `/api/v3/historical-price-full/{symbol}` | All indicators, relative strength |
| Earnings calendar | `get_earnings_calendar` | `/api/v3/earning_calendar` | Earnings blackout rule, catalyst timing |
| Economic calendar | `get_economic_calendar` | `/api/v3/economic_calendar` | Macro events, risk events |
| Company news | `get_company_news` | `/api/v3/stock_news` | Catalyst identification |
| Sector performance | `get_sector_performance` | `/api/v3/sector-performance` | Sector bias |
